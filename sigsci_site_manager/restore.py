...
def restore_rule_lists(api, data):
    print('Merging lists...')
    ...
    for item in data:
        ...
        if existing:
            ...
            #_restore_lists(api, item, existing)
...
def restore_custom_signals(api, data):
...
def restore_request_rules(api, data):
...
def restore_site_rules(api, data):
...
def restore_signal_rules(api, data):
...
def restore_templated_rules(api, data):
...
def restore_custom_alerts(api, data):
    print('Restoring custom alerts...')
    ...
    # Delete all current alerts
    for alert in alerts:
        if alert['action'] != 'siteMetricInfo':
            print('  Deleting %s' % alert['longName'])
            api.delete_custom_alert(alert['id'])
...
def restore_site_members(api, data):
    print('Merging users...')
...
def restore_integrations(api, data):
...
def restore_advanced_rules(api, source, data):
...
def restores(api, site_name, data, categories):
    ...
    steps = OrderedDict()
    steps[RULE_LISTS] = (
        restore_rule_lists, (api, data['rule_lists'])
    )
    steps[CUSTOM_SIGNALS] = (
        restore_custom_signals, (api, data['custom_signals'])
    )
    steps[TEMPLATED_RULES] = (
        restore_templated_rules, (api, data['templated_rules'])
    )
    steps[CUSTOM_ALERTS] = (
        restore_custom_alerts, (api, data['custom_alerts'])
    )
    steps[SITE_MEMBERS] = (
        restore_site_members, (api, data['site_members'])
    )
    steps[INTEGRATIONS] = (
        restore_integrations, (api, data['integrations'])
    )
    steps[ADVANCED_RULES] = (
        restore_advanced_rules,
    ...
    if 'site_rules' in data:
        steps[SITE_RULES] = (
            restore_site_rules, (api, data['site_rules'])
        )
    else:
        steps[REQUEST_RULES] = (
            restore_request_rules, (api, data['request_rules'])
        )
        steps[SIGNAL_RULES] = (
            restore_signal_rules, (api, data['signal_rules'])
        )
...
def restore(api, dst_site, file_name, categories=None):
    print("Restoring file '%s' to site '%s'..." % (file_name, dst_site))
    with open(file_name, 'r') as f:
        data = json.loads(f.read())

    restores(api, dst_site, data, categories)
